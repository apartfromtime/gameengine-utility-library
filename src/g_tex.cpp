    ...
    pixel_t srcformat)
{
    ...
    while (y < dstysize)
    {
        ...
        while (x < dstxsize)
        {
            ...
            case PIXELTYPE_RGB:
            {
                bufdst[x*3+0] = (pixel & rmask);
                bufdst[x*3+1] = (pixel & gmask);
                bufdst[x*3+2] = (pixel & bmask);
            } break;
            case PIXELTYPE_BGR:
            {
                bufdst[x*3+0] = (pixel & bmask);
                bufdst[x*3+1] = (pixel & gmask);
                bufdst[x*3+2] = (pixel & rmask);
            } break;
            case PIXELTYPE_XBGR1555:
            {
                r = (pixel & rmask) * 0x1F / 0xFF;
                g = (pixel & gmask) * 0x1F / 0xFF;
                b = (pixel & bmask) * 0x1F / 0xFF;
            ...
            case PIXELTYPE_LUMINANCE_ALPHA:
            {
                bufdst[x*2+0] = (uint8_t)((pixel & rmask) * 0.2990f +
                    (pixel & gmask) * 0.5870f +
                    (pixel & bmask) * 0.1140f);
    ...
    pixel_t srcformat)
{
    ...
    while (y < dstysize)
    {
        ...
        while (x < dstxsize)
        {
            ...
            case PIXELTYPE_RGBA:
            {
                bufdst[x*4+0] = (pixel & rmask);
                bufdst[x*4+1] = (pixel & gmask);
                bufdst[x*4+2] = (pixel & bmask);
                bufdst[x*4+3] = (pixel & amask);
            } break;
            case PIXELTYPE_ABGR:
            {
                bufdst[x*4+0] = (pixel & amask);
                bufdst[x*4+1] = (pixel & bmask);
                bufdst[x*4+2] = (pixel & gmask);
                bufdst[x*4+3] = (pixel & rmask);
            } break;
            case PIXELTYPE_BGRA:
            {
                bufdst[x*4+0] = (pixel & bmask);
                bufdst[x*4+1] = (pixel & gmask);
                bufdst[x*4+2] = (pixel & rmask);
                bufdst[x*4+3] = (pixel & amask);
            } break;
            case PIXELTYPE_XBGR1555:
            {
    ...
    pixel_t srcformat)
{
    ...
    while (y < dstysize)
    {
        ...
        while (x < dstxsize)
        {
            ...
            case PIXELTYPE_RGBA:
            {
                bufdst[x*4+0] = (pixel & rmask) * rmod;
                bufdst[x*4+1] = (pixel & gmask) * gmod;
                bufdst[x*4+2] = (pixel & bmask) * bmod;
                bufdst[x*4+3] = (pixel & amask) * amod;
            } break;
            case PIXELTYPE_ABGR:
            {
                bufdst[x*4+0] = (pixel & amask) * amod;
                bufdst[x*4+1] = (pixel & bmask) * bmod;
                bufdst[x*4+2] = (pixel & gmask) * gmod;
                bufdst[x*4+3] = (pixel & rmask) * rmod;
            } break;
            case PIXELTYPE_BGRA:
            {
                bufdst[x*4+0] = (pixel & bmask) * bmod;
                bufdst[x*4+1] = (pixel & gmask) * gmod;
                bufdst[x*4+2] = (pixel & rmask) * rmod;
                bufdst[x*4+3] = (pixel & amask) * amod;
            } break;
            case PIXELTYPE_RGB:
            {
                bufdst[x*3+0] = (pixel & rmask) * rmod;
                bufdst[x*3+1] = (pixel & gmask) * gmod;
                bufdst[x*3+2] = (pixel & bmask) * bmod;
            } break;
            case PIXELTYPE_BGR:
            {
                bufdst[x*3+0] = (pixel & bmask) * bmod;
                bufdst[x*3+1] = (pixel & gmask) * gmod;
                bufdst[x*3+2] = (pixel & rmask) * rmod;
            } break;
            }
...
//-----------------------------------------------------------------------------
void
PointFilterNbitsTo32BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
    uint8_t* rawdst = pdst;
    uint8_t* rawsrc = psrc;
    ...
    while (y < dstysize)
    {
        ...
        while (x < dstxsize)
        {
            ...
            {
                rgb1555 = 0;
                rgb1555 |= bufsrc[x0*2+0];
                rgb1555 |= bufsrc[x0*2+1] << 8;
...
//-----------------------------------------------------------------------------
// PointFilterNbitsTo24BPP
//-----------------------------------------------------------------------------
void
PointFilterNbitsTo24BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
...
//-----------------------------------------------------------------------------
// PointFilterNbitsTo16BPP
//-----------------------------------------------------------------------------
void
PointFilterNbitsTo16BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
...
//-----------------------------------------------------------------------------
void
LinearFilterNbitsTo32BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
    ...
    while (y < dstysize)
    {
        ...
        while (x < dstxsize)
        {
            ...
            case PIXELTYPE_RGBA:
            {
                rx0y0 = bufsrc0[x0*4+0];
                gx0y0 = bufsrc0[x0*4+1];
                bx0y0 = bufsrc0[x0*4+2];
                ax0y0 = bufsrc0[x0*4+3];

                rx1y0 = bufsrc0[x1*4+0];
                gx1y0 = bufsrc0[x1*4+1];
                bx1y0 = bufsrc0[x1*4+2];
                ax1y0 = bufsrc0[x1*4+3];

                rx0y1 = bufsrc1[x0*4+0];
                gx0y1 = bufsrc1[x0*4+1];
                bx0y1 = bufsrc1[x0*4+2];
                ax0y1 = bufsrc1[x0*4+3];

                rx1y1 = bufsrc1[x1*4+0];
                gx1y1 = bufsrc1[x1*4+1];
                bx1y1 = bufsrc1[x1*4+2];
                ax1y1 = bufsrc1[x1*4+3];
            } break;
            case PIXELTYPE_ABGR:
            {
                ax0y0 = bufsrc0[x0*4+0];
                bx0y0 = bufsrc0[x0*4+1];
                gx0y0 = bufsrc0[x0*4+2];
                rx0y0 = bufsrc0[x0*4+3];

                ax1y0 = bufsrc0[x1*4+0];
                bx1y0 = bufsrc0[x1*4+1];
                gx1y0 = bufsrc0[x1*4+2];
                rx1y0 = bufsrc0[x1*4+3];

                ax0y1 = bufsrc1[x0*4+0];
                bx0y1 = bufsrc1[x0*4+1];
                gx0y1 = bufsrc1[x0*4+2];
                rx0y1 = bufsrc1[x0*4+3];

                ax1y1 = bufsrc1[x1*4+0];
                bx1y1 = bufsrc1[x1*4+1];
                gx1y1 = bufsrc1[x1*4+2];
                rx1y1 = bufsrc1[x1*4+3];
            } break;
            case PIXELTYPE_BGRA:
            {
                bx0y0 = bufsrc0[x0*4+0];
                gx0y0 = bufsrc0[x0*4+1];
                rx0y0 = bufsrc0[x0*4+2];
                ax0y0 = bufsrc0[x0*4+3];

                bx1y0 = bufsrc0[x1*4+0];
                gx1y0 = bufsrc0[x1*4+1];
                rx1y0 = bufsrc0[x1*4+2];
                ax1y0 = bufsrc0[x1*4+3];

                bx0y1 = bufsrc1[x0*4+0];
                gx0y1 = bufsrc1[x0*4+1];
                rx0y1 = bufsrc1[x0*4+2];
                ax0y1 = bufsrc1[x0*4+3];

                bx1y1 = bufsrc1[x1*4+0];
                gx1y1 = bufsrc1[x1*4+1];
                rx1y1 = bufsrc1[x1*4+2];
                ax1y1 = bufsrc1[x1*4+3];
            } break;
            case PIXELTYPE_RGB:
            {
                rx0y0 = bufsrc0[x0*3+0];
                gx0y0 = bufsrc0[x0*3+1];
                bx0y0 = bufsrc0[x0*3+2];
                ax0y0 = a;

                rx1y0 = bufsrc0[x1*3+0];
                gx1y0 = bufsrc0[x1*3+1];
                bx1y0 = bufsrc0[x1*3+2];
                ax1y0 = a;

                rx0y1 = bufsrc1[x0*3+0];
                gx0y1 = bufsrc1[x0*3+1];
                bx0y1 = bufsrc1[x0*3+2];
                ax0y1 = a;

                rx1y1 = bufsrc1[x1*3+0];
                gx1y1 = bufsrc1[x1*3+1];
                bx1y1 = bufsrc1[x1*3+2];
                ax1y1 = a;
            } break;
            case PIXELTYPE_BGR:
            {
                bx0y0 = bufsrc0[x0*3+0];
                gx0y0 = bufsrc0[x0*3+1];
                rx0y0 = bufsrc0[x0*3+2];
                ax0y0 = a;

                bx1y0 = bufsrc0[x1*3+0];
                gx1y0 = bufsrc0[x1*3+1];
                rx1y0 = bufsrc0[x1*3+2];
                ax1y0 = a;

                bx0y1 = bufsrc1[x0*3+0];
                gx0y1 = bufsrc1[x0*3+1];
                rx0y1 = bufsrc1[x0*3+2];
                ax0y1 = a;

                bx1y1 = bufsrc1[x1*3+0];
                gx1y1 = bufsrc1[x1*3+1];
                rx1y1 = bufsrc1[x1*3+2];
                ax1y1 = a;
            } break;
            case PIXELTYPE_XBGR1555:
            {
            ...
            case PIXELTYPE_LUMINANCE_ALPHA:
            {
                rx0y0 = bufsrc0[x0*2+0];
                gx0y0 = rx0y0;
                bx0y0 = rx0y0;
                ax0y0 = bufsrc0[x0*2+1];

                rx1y0 = bufsrc0[x1*2+0];
                gx1y0 = rx1y0;
                bx1y0 = rx1y0;
                ax1y0 = bufsrc0[x1*2+1];

                rx0y1 = bufsrc1[x0*2+0];
                gx0y1 = rx0y1;
                bx0y1 = rx0y1;
                ax0y1 = bufsrc1[x0*2+1];

                rx1y1 = bufsrc1[x1*2+0];
                gx1y1 = rx1y1;
                bx1y1 = rx1y1;
                ax1y1 = bufsrc1[x1*2+1];
            } break;
            case PIXELTYPE_LUMINANCE:
            {
                rx0y0 = bufsrc0[x0*1+0];
                gx0y0 = rx0y0;
                bx0y0 = rx0y0;
                ax0y0 = a;

                rx1y0 = bufsrc0[x1*1+0];
                gx1y0 = rx1y0;
                bx1y0 = rx1y0;
                ax1y0 = a;

                rx0y1 = bufsrc1[x0*1+0];
                gx0y1 = rx0y1;
                bx0y1 = rx0y1;
                ax0y1 = a;

                rx1y1 = bufsrc1[x1*1+0];
                gx1y1 = rx1y1;
                bx1y1 = rx1y1;
                ax1y1 = a;
            } break;
...
//-----------------------------------------------------------------------------
// LinearFilterNbitsTo24BPP
//-----------------------------------------------------------------------------
void
LinearFilterNbitsTo24BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
...
//-----------------------------------------------------------------------------
// LinearFilterNbitsTo16BPP
//-----------------------------------------------------------------------------
void
LinearFilterNbitsTo16BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
...
//-----------------------------------------------------------------------------
void
BlitNbitsTo32BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
    ...
    while (y < ysize)
    {
        ...
        while (x < xsize)
        {
            ...
            case PIXELTYPE_RGBA:
            {
                r = bufsrc[(x%srcxsize)*4+0];
                g = bufsrc[(x%srcxsize)*4+1];
                b = bufsrc[(x%srcxsize)*4+2];
                a = bufsrc[(x%srcxsize)*4+3];
            } break;
            case PIXELTYPE_ABGR:
            {
                a = bufsrc[(x%srcxsize)*4+0];
                b = bufsrc[(x%srcxsize)*4+1];
                g = bufsrc[(x%srcxsize)*4+2];
                r = bufsrc[(x%srcxsize)*4+3];
            } break;
            case PIXELTYPE_BGRA:
            {
                b = bufsrc[(x%srcxsize)*4+0];
                g = bufsrc[(x%srcxsize)*4+1];
                r = bufsrc[(x%srcxsize)*4+2];
                a = bufsrc[(x%srcxsize)*4+3];
            } break;
            case PIXELTYPE_RGB:
            {
                r = bufsrc[(x%srcxsize)*3+0];
                g = bufsrc[(x%srcxsize)*3+1];
                b = bufsrc[(x%srcxsize)*3+2];
            } break;
            case PIXELTYPE_BGR:
            {
                b = bufsrc[(x%srcxsize)*3+0];
                g = bufsrc[(x%srcxsize)*3+1];
                r = bufsrc[(x%srcxsize)*3+2];
            } break;
            case PIXELTYPE_XBGR1555:
            {
                rgb1555 = 0;
                rgb1555 |= bufsrc[(x%srcxsize)*2+0];
                rgb1555 |= bufsrc[(x%srcxsize)*2+1] << 8;
            ...
            case PIXELTYPE_LUMINANCE_ALPHA:
            {
                r = bufsrc[(x%srcxsize)*2+0];
                g = r;
                b = r;
                a = bufsrc[(x%srcxsize)*2+1];
            } break;
            case PIXELTYPE_LUMINANCE:
            {
                r = bufsrc[(x%srcxsize)*1+0];
                g = r;
                b = r;
            } break;
            case PIXELTYPE_COLOUR_INDEX:
...
//-----------------------------------------------------------------------------
void
BlitNbitsTo24BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
...
//-----------------------------------------------------------------------------
void
BlitNbitsTo16BPP(uint8_t* pdst, uint32_t dstpitch, uint32_t dstxsize,
    uint32_t dstysize, pixel_t dstformat, uint8_t* psrc, uint32_t srcpitch,
    uint32_t srcxsize, uint32_t srcysize, pixel_t srcformat,
    palette_t* ppalette)
{
...
//-----------------------------------------------------------------------------
bool
ResampleImage(uint8_t** ppdst, rect_t dstrect, pixel_t dstformat,
    palette_t* ppalette, image_t* psrcimage, rect_t srcrect, uint32_t filter)
{
    if (ppdst == NULL || psrcimage == NULL)
    {
        return false;
    }

    // dst stuff
    int32_t dstxorigin = dstrect.min[0] < 0 ? 0 : dstrect.min[0];
    int32_t dstyorigin = dstrect.min[1] < 0 ? 0 : dstrect.min[1];
    int32_t dstxextent = dstrect.max[0]-dstrect.min[0];
    int32_t dstyextent = dstrect.max[1]-dstrect.min[1];
    ...
    uint8_t* dstbuf = ppdst[0] + (dstyorigin * dstpitch) +
        (dstxorigin * dstbytesperpixel);            // start of current row

    // src stuff
    int32_t srcxorigin = srcrect.min[0] < 0 ? 0 : srcrect.min[0];
    int32_t srcyorigin = srcrect.min[1] < 0 ? 0 : srcrect.min[1];
    int32_t srcxextent = srcrect.max[0]-srcrect.min[0];
    int32_t srcyextent = srcrect.max[1]-srcrect.min[1];
    ...
    rect_t* psrcrect)
{
    ...
    if (psrcimage != NULL)
    {
        ...
        if (psrcrect == NULL)
        {
            rect.min[0] = 0;
            rect.min[1] = 0;
            rect.max[0] = psrcimage->xsize;
            rect.max[1] = psrcimage->ysize;
        }
        else
        {
            rect.min[0] = ABS(psrcrect->min[0]);
            rect.min[1] = ABS(psrcrect->min[1]);
            rect.max[0] = ABS(psrcrect->max[0]);
            rect.max[1] = ABS(psrcrect->max[1]);
        }

        if ((uint32_t)rect.min[0] >= psrcimage->xsize)
        {
            fprintf(stderr, "SaveImage, Invalid rectangle x-origin.\n");
            return false;
        }

        if ((uint32_t)rect.min[1] >= psrcimage->ysize)
        {
            fprintf(stderr, "SaveImage, Invalid rectangle y-origin.\n");
            return false;
        }

        if ((uint32_t)rect.max[0] > psrcimage->xsize)
        {
            fprintf(stderr, "SaveImage, Invalid rectangle width.\n");
            return false;
        }

        if ((uint32_t)rect.max[1] > psrcimage->ysize)
        {
            fprintf(stderr, "SaveImage, Invalid rectangle height.\n");
            return false;
        }

        xsize = rect.max[0]-rect.min[0];
        ysize = rect.max[1]-rect.min[1];
        ...
        pixels = (uint8_t*)malloc(xsize * ysize * (depth >> 3));
        memset(pixels, 0, xsize * ysize * (depth >> 3));

        // convert to dst format
        result = ResampleImage(&pixels, rect, format, psrcpalette, psrcimage,
            rect, FILTER_NONE);

        if (format != PIXELTYPE_COLOUR_INDEX)
        {
            psrcpalette = NULL;
        }
        ...
        if (result == true)
        {
            ...
            case FILEFORMAT_PNG:
            {
                result = SaveToMemoryPNG(ppdst, ppdstsize, dstcodec, pixels,
                    xsize, ysize, depth, psrcpalette, NULL);
            } break;
            case FILEFORMAT_BMP:
            {
                result = SaveToMemoryBMP(ppdst, ppdstsize, dstcodec, pixels,
                    xsize, ysize, depth, psrcpalette, NULL, false);
            } break;
            case FILEFORMAT_PCX:
            {
                result = SaveToMemoryPCX(ppdst, ppdstsize, dstcodec, pixels,
                    xsize, ysize, depth, psrcpalette);
            } break;
            case FILEFORMAT_TGA:
            {
                result = SaveToMemoryTGA(ppdst, ppdstsize, dstcodec, pixels,
                    xsize, ysize, depth, psrcpalette, false,
                    false);
            } break;
            default:
            {
    ...
    image_info_t* psrcinfo)
{
    ...
    if (pdstimage != NULL)
    {
        ...
        if (pdstrect == NULL)
        {
            rectdst.min[0] = 0;
            rectdst.min[1] = 0;
            rectdst.max[0] = srcimage.xsize;
            rectdst.max[1] = srcimage.ysize;
        }
        else
        {
            rectdst = *pdstrect;
        ...
        if (rectdst.min[0] >= MAX_DIMENSION)
        {
            fprintf(stderr, "LoadImage, Invalid dst rectangle x-origin.\n");
            result = false;
        }

        if (rectdst.min[1] >= MAX_DIMENSION)
        {
            fprintf(stderr, "LoadImage, Invalid dst rectangle y-origin.\n");
            result = false;
        }

        if (rectdst.max[0] > MAX_DIMENSION)
        {
            fprintf(stderr, "LoadImage, Invalid dst rectangle x-extent.\n");
            result = false;
        }

        if (rectdst.max[1] > MAX_DIMENSION)
        {
            fprintf(stderr, "LoadImage, Invalid dst rectangle y-extent.\n");
            result = false;
        }
        ...
        if (psrcrect == NULL)
        {
            rectsrc.min[0] = 0;
            rectsrc.min[1] = 0;
            rectsrc.max[0] = srcimage.xsize;
            rectsrc.max[1] = srcimage.ysize;
        }
        else
        {
            rectsrc = *psrcrect;
        ...
        if ((uint32_t)rectsrc.min[0] >= srcimage.xsize)
        {
            fprintf(stderr, "LoadImage, Invalid src rectangle x-origin.\n");
            result = false;
        }

        if ((uint32_t)rectsrc.min[1] >= srcimage.ysize)
        {
            fprintf(stderr, "LoadImage, Invalid src rectangle y-origin.\n");
            result = false;
        }

        if ((uint32_t)rectsrc.max[0] > srcimage.xsize)
        {
            fprintf(stderr, "LoadImage, Invalid src rectangle x-extent.\n");
            result = false;
        }

        if ((uint32_t)rectsrc.max[1] > srcimage.ysize)
        {
            fprintf(stderr, "LoadImage, Invalid src rectangle y-extent.\n");
            result = false;
        }
        ...
        {            
            ...
            result = ResampleImage(&pdstimage->data, rectdst, dstformat,
                &srcpalette, &srcimage, rectsrc, filter);
            ...
            if (result == true)
            {
                ...
                pdstimage->xsize = rectdst.max[0]-rectdst.min[0];
                pdstimage->ysize = rectdst.max[1]-rectdst.min[1];
