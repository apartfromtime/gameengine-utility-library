    ...
    uint8_t* psrc)
{
    ...
    uint32_t srcxskip = srcbytes;
    uint32_t srcyskip = srcpitch;
    uint32_t rawxskip = srcxskip;
    ...
    bufptr = (uint8_t*)malloc((xsize + 8) * srcbytes);
    buffer = bufptr;
    ...
    for (uint32_t pass = 0; pass < passes; ++pass)
    {
        if (interlaced)
        {
            ...
            srcbuf = psrc + (i_yorigin[pass] * srcpitch) + (i_xorigin[pass] * srcbytes);
            srcxskip = i_xextent[pass] * srcbytes;
            srcyskip = i_yextent[pass] * srcpitch;
            rawxskip = srcxskip;

            dstbuf = pdst + bytesencoded;
        }
        ...
        while (y < ysize)
        {
                ...
                for (int f = 0; f < PNG_FILTER_COUNT - 1; ++f)
                {
                    ...
                    while (x < xsize)
                    {
                        ...
                        while (bpp < srcbytes)
                        {
                            if (y != 0 && x != 0)
                            {
                                pri1 = (rawbuf - srcyskip - srcxskip)[bpp];
                            }

                            if (y != 0)
                            {
                                pri0 = (rawbuf - srcyskip)[bpp];
                            }

                            if (x != 0)
                            {
                                raw1 = (rawbuf - srcxskip)[bpp];
                            }

                            raw0 = sample = rawbuf[bpp];
            ...
            memset(bufptr, 0, (xsize + 8) * srcbytes);
            buffer = bufptr;
            ...
            *dstbuf++ = enc;           // filter-type byte
            ...
            {
                int32_t rem = xsize;
                int32_t count = PIXELS_PER_BYTE(srcdepth);

                while (rem > 0)
                {
                    if (rem < count) { count = rem; }
                    ShrinkIndex8ToNbits(dstbuf, buffer, 1, count, (8 - srcdepth));
                    buffer += count;
                    dstbuf++;
                    bytesencoded++;
                    rem -= count;
    ...
    rgba_t* pcolorkey)
{
    ...
    *dstbuf++ = interlace;
    ...
    {
        ShrinkPNG(idatbuf, &idatlen, dstxsize, dstysize, srcdepth, interlace, filtermode, rawbuf);
    }
            ...
            while (x < xsize)
            {
                ...
                while (bpp < dstbytes)
                {
                    ...
                    if (dstdepth < 8)
                    {
                        if ((x1 >= 0) && (y1 >= 0))
                        {                     
                            bit = (x1 - ((uint32_t)(x1 / dstpixelsperbyte) *
                                dstpixelsperbyte)) * dstdepth;
                            pixel = (pixbuf + bpp) + (y1 * dstpitch) +
                                (uint32_t)(x1 / dstpixelsperbyte);
                            pri1 = (*pixel>>((8-dstdepth)-bit))&mask[dstdepth-1];
                        }
                        ...
                        if ((x1 >= 0))
                        {
                            bit = (x1 - ((uint32_t)(x1 / dstpixelsperbyte) *
                                dstpixelsperbyte)) * dstdepth;
                            pixel = (pixbuf + bpp) + (y0 * dstpitch) +
                                (uint32_t)(x1 / dstpixelsperbyte);
                            raw1 = (*pixel>>((8-dstdepth)-bit))&mask[dstdepth-1];
                        }

                        if ((y1 >= 0))
                        {
                            bit = (x0 - ((uint32_t)(x0 / dstpixelsperbyte) *
                                dstpixelsperbyte)) * dstdepth;
                            pixel = (pixbuf + bpp) + (y1 * dstpitch) +
                                (uint32_t)(x0 / dstpixelsperbyte);
                            pri0 = (*pixel>>((8-dstdepth)-bit))&mask[dstdepth-1];
                        }
                    }
                    else
                    {
                        if ((x1 >= 0) && (y1 >= 0))
                        {
                            pri1 = *((pixbuf + bpp) + (y1 * dstpitch) +
                                (x1 * dstbytes));
                        }

                        if ((x1 >= 0))
                        {
                            raw1 = *((pixbuf + bpp) + (y0 * dstpitch) +
                                (x1 * dstbytes));
                        }

                        if ((y1 >= 0))
                        {
                            pri0 = *((pixbuf + bpp) + (y1 * dstpitch) +
                                (x0 * dstbytes));
                        }
                    }
                    ...
                    if (dstdepth < 8)
                    {
                        bit = (x0 - ((uint32_t)(x0 / dstpixelsperbyte) *
                            dstpixelsperbyte)) * dstdepth;
                        (pixbuf + (((y * dstyskip) + dstyorig) * dstpitch) +
                            (uint32_t)(x0 / dstpixelsperbyte))[bpp] |=
                                (((sample >> ((pix - cur) * dstdepth)) &
                                    mask[dstdepth - 1]) & mask[dstdepth - 1]) <<
                                        ((8 - dstdepth) - bit);
                    }
                    else
                    {
